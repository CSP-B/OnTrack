...
import bjoernercomplete.diagram.edit.commands.LinearCreateCommand;
...
public class RailDiagramItemSemanticEditPolicy extends
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (BjoernerCompleteElementTypes.Connector_2001 == req.getElementType()) {
			return getGEFWrapper(new ConnectorCreateCommand(req));
		}
		if (BjoernerCompleteElementTypes.Linear_2002 == req.getElementType()) {
			return getGEFWrapper(new LinearCreateCommand(req));
		}
		if (BjoernerCompleteElementTypes.Signal_2003 == req.getElementType()) {
			return getGEFWrapper(new SignalCreateCommand(req));
		}
		if (BjoernerCompleteElementTypes.Point_2004 == req.getElementType()) {
			return getGEFWrapper(new PointCreateCommand(req));
		}
		if (BjoernerCompleteElementTypes.ControlTable_2005 == req
				.getElementType()) {
